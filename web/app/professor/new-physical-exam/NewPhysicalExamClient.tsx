...
import { savePhysicalExam } from './actions';

export default function NewPhysicalExamClient({ userName }: { userName: string }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const result = await savePhysicalExam({
        title,
        description,
        aiInstructions,
        totalScore,
        answerKeyImages: imageUrls,
      });
        ...
        <div className="p-4 sm:p-6 md:p-8 lg:p-12 max-w-[1200px] mx-auto space-y-8 animate-in fade-in slide-in-from-top-4 duration-700">
          ...
          <header className="flex items-center justify-between">
            <div className="space-y-1">
              ...
              <p className="text-gray-400">Configure a prova física para que a IA possa corrigir as provas dos alunos depois.</p>
            </div>
            ...
            <div className="space-y-6">
              <section className="liquid-glass p-8 rounded-3xl border border-outline-variant space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <span className="bg-primary/20 text-primary px-3 py-1 rounded-full text-sm font-bold">{files.length} folha(s)</span>
                </div>
                <p className="text-sm text-gray-400">Tire fotos claras do gabarito oficial ou faça o upload das imagens. Adicione uma folha por vez se necessário.</p>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {files.map((file, i) => (
                    <div key={i} className="relative aspect-[3/4] bg-white/5 border border-white/10 rounded-2xl overflow-hidden group">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-md p-2 text-xs truncate">
                        Folha {i + 1}
                      </div>
                    </div>
                  ))}
                  ...
                  >
                    ...
                    <span className="font-bold text-sm">+ Adicionar mais folhas</span>
                  </button>
                  <input 
                    type="file" 
                    accept="image/*" 
